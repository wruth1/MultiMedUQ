...
TMB_2_GLMM_pars_list <- function(fit_TMB) {
    ...
    TMB_FE_pars = TMB_pars$TMB_FE_pars
    ...
    output = list(TMB_FE_pars = TMB_FE_pars, TMB_SDs = TMB_SDs, TMB_corrs = TMB_corrs)
    return(output)
}
...
# Staple together the three groups of gradients and fix the order on the RE pars
...
# SE matrix from TMB for its own parameterization
...
# Delta method adjustment for TMB SEs from their parameterization to mine
